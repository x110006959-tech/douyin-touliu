...
type Account = { id: string; accountName: string; platformAccountId: string | null };
...
function NewProjectForm() {
  ...
  return <main className="mx-auto max-w-3xl px-6 py-8"><Link className="text-sm text-primary" href={`/accounts/${account.id}`}>返回账号档案</Link><Card className="mt-4"><CardTitle>{source ? "复用上次基础信息创建新项目" : "创建账号诊断项目"}</CardTitle><div className="mb-5 rounded-md border border-border bg-slate-50 p-3 text-sm"><strong>当前账号：{account.accountName}</strong><p className="mt-1 text-muted">{account.platformAccountId ? `账号 ID：${account.platformAccountId}` : "账号 ID 待补充；采集时无法识别账号的数据将进入人工确认。"}</p>{source ? <p className="mt-2 text-muted">已从“{source.name}”预填基础配置，你可以按本次活动调整。提交后只创建新项目，不修改源项目，也不复制快照、诊断和动作记录。</p> : null}</div><form className="grid gap-4" key={sourceProjectId || account.id} onSubmit={submit}>
