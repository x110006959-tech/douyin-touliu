...
import { subjectTypeLabels, type SubjectType } from "@douyin-local-life/shared";
...
type Account = { id: string; accountName: string; merchantName: string | null; storeName: string | null; memo: string | null; projects: Array<{ id: string; name: string; subjectType: SubjectType; status: string; updatedAt: string; _count: { tasks: number } }> };
...
export default function AccountPage() {
  ...
  async function save(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await apiFetch(`/account-profiles/${account.id}`, token, { method: "PATCH", body: JSON.stringify({ accountName: form.get("accountName"), merchantName: form.get("merchantName") || null, storeName: form.get("storeName") || null, memo: form.get("memo") || null }) });
      setMessage("账号档案已保存；已配对插件继续按服务端任务归属安全校验。"); load();
