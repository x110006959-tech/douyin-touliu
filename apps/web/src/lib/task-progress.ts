...
export function getTaskWizardProgress(input: TaskWizardProgressInput) {
  const steps = [
    ...
    { number: 3, label: "账号与数据汇总", complete: input.requiredRoutesAccountMatched },
    { number: 4, label: "人工核对", complete: input.requiredRoutesAccountMatched && input.reviewComplete },
